...
class NavigationService {
  ...
  initialiseRoutes() async {
    try {
      ...
      if (jsonString == null || jsonString.length == 0) {
        throw "Navigation Service: No data available";
      }
      Map<String, dynamic> json = jsonDecode(jsonString);
      _deliveryRoutes = DeliveryRoute.fromJson(json);
    }
    catch (error) {
      print("Error initialising routes from json file.[$error]");
      return;
    }
  }
  ...
  initialisePolyPointsAndMarkers(int route) async {
    try{
      ...
      print("Error initialising polylines.[$error]");
      return;
    }
  }
...
//__________________________________________________________________________________________________
    ...
    try {
      ...
      while (currentPolyline.points.length > 0 &&
        ...
        if (_currentStep + 1 <= _deliveryRoutes.routes[_currentRoute].legs[_currentLeg].steps.length - 1
            &&
            calculateDistanceBetween(currentPolyline.points[0], getStepStartLatLng(_currentRoute, _currentLeg, _currentStep + 1)
            ) < 20) {
                  _currentStep += 1;

        } else if (_currentStep - 1 >= 0 && _currentStep + 1 <=
                _deliveryRoutes.routes[_currentRoute].legs[_currentLeg].steps.length - 1
            &&
            (calculateDistanceBetween(currentPolyline.points[0],
                getStepEndLatLng(_currentRoute, _currentLeg, _currentStep - 1)
            ) < 20)) {
          _currentStep += 1;
        }
        currentPolyline.points.removeAt(0);
  ...
  updateCurrentDeliveryRoutes() async {
    /*
    TODO
      - remove this and change the way delivery screen works. with selected route method display
     */
  ...
  notifyInfoChange(){
    ...
    /*
    TODO
      -create notify methods for all the vars below
     */
    String directions;
    String deliveryTimeRemaining;
    int distance;
  ...
  Future<int> getChosenRoute() async {
    String route = await _storage.read(key: 'current_route');
    if(route != null){
      return int.parse(route);
    }
    return -1;
  }
   ...
   * Parameters: none
   * Returns: int
   * Description: Returns the number of points after the next delivery point
   * since the points gets popped off from front of the polyline points list
   * as the driver travels over the points.
   */
  ...
  int calculateNextStepPoint(){
    /*
    TODO
      - using same method above calculate next step point
     */

    LatLng nextStepStart = getStepStartLatLng(_currentRoute, _currentLeg, _currentStep + 1);
    LatLng currentStepEnd = getStepEndLatLng(_currentRoute, _currentLeg, _currentStep);
    ...
    for (int i = 0; i < currentPolyline.points.length; i++) {
      ...
      if (_currentStep + 1 <= currentPolyline.points.length - 1 &&
          calculateDistanceBetween(
              ...
              nextStepStart) <
              20) {
        return currentPolyline.points.length - i;
  ...
  navigate(Position currentPosition, BuildContext context) async {
    ...
    /*
    TODO
      - redo to fit new methods and observers
      - make wrapper function for all pre-checks
      -
     */
    if (_deliveryRoutes == null) {
      initialiseRoutes();
