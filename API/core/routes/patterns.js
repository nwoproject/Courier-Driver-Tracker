...
router.post('/monthly/:driverid', async (req,res)=>{
    const driverID =  req.params.driverid;
    const datetime =  new Date(new Date() + 'GMT');
    const patternDate = datetime.toISOString().slice(0,10);
    ...
    if(!res.writableEnded)
    {
        DB.pool.query('INSERT INTO ai."monthly_pattern"("pattern_detected","abnormality","week","date","driver_id")VALUES($1,$2,$3,$4,$5)',
        [req.body.patternsDetected,req.body.abnormalities,req.body.weeks,patternDate,driverID],(err,results)=>{
            if(err)
            {
                DB.dbErrorHandler(res,err);
            }
